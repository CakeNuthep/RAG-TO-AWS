...
export class RagCdkInfraStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Lambda function (image) to handle the Line worker logic (run RAG/AI model).
    const lineWorkerImageCode = DockerImageCode.fromImageAsset("../image", {
      cmd: ["line_work_handler.handler"],
      buildArgs: {
        platform: "linux/amd64", // Needs x86_64 architecture for pysqlite3-binary.
      },
    });
    const lineWorkerFunction = new DockerImageFunction(this, "RagLineWorkerFunction", {
      code: lineWorkerImageCode,
    ...
    const apiFunction = new DockerImageFunction(this, "ApiFunc", {
      ...
      environment: {
        TABLE_NAME: ragQueryTable.tableName, 
        LINE_WORKER_LAMBDA_NAME: lineWorkerFunction.functionName, 
      },
    });
    ...
    ragQueryTable.grantReadWriteData(lineWorkerFunction);
    ragQueryTable.grantReadWriteData(apiFunction);
    lineWorkerFunction.grantInvoke(apiFunction);
    lineWorkerFunction.role?.addManagedPolicy(
